...
public class SymbolMapper {
    ...
    private final static HashMap<String, String> unicode_to_ascii = new HashMap<>();
    private final static HashMap<String, String> ascii_to_smiley = new HashMap<>();
    ...
    private final static String[][] SMILEY_MAPPING = {
            {":mad:", ">:("},
            {":blush:", ":\")"},
            {":broken_heart:", "</3"},
            {":confused:", ":-/"},
    ...
    static {
        for (String[] s : UNICODE_MAPPING) {
            unicode_to_ascii.put(s[1], s[0]);
        }
        for (String[] s : SMILEY_MAPPING) {
            ascii_to_smiley.put(s[0], s[1]);
        }
    }

    public static String replaceSymbols(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            String ascii = unicode_to_ascii.get(key);
            if (ascii != null) {
                String smiley = ascii_to_smiley.get(ascii);
                if (smiley != null) {
                    s = s.replaceAll(key, smiley);
                } else {
                    s = s.replaceAll(key, ascii);
                }
            }
        }
        return s;
